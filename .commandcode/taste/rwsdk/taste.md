...
- Make SEO/OG tags content-driven: source per-page OG title/description/image (and alt text) from content frontmatter (e.g., `ogImage`/`ogImageAlt` fields on a podcast document) with fallback to site defaults (`/og-image.png`), and include the content-driven image in JSON-LD too. Confidence: 0.9
- Cache-bust content images so they're easy to update: add a single version field to the content frontmatter (e.g., `imagesVersion: z.number().default(1)`), then append `?v=N` to every image path centrally in the collection's `transform` (handling URLs that already have query strings) — refreshing assets is just bumping one number instead of editing every reference. Confidence: 0.85
- Name page files descriptively after their route rather than using `index.tsx` or bracket-style dynamic filenames (e.g., `pages/podcasts/podcasts.tsx` for the list and `pages/podcasts/podcast.tsx` for the detail) — routes are wired explicitly in `worker.tsx`, so file names should stay human-readable. Confidence: 0.7
